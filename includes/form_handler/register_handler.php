...
if (isset($_POST['register_button'])) {
    ...
    $fname = ucfirst(strtolower($fname)); //convert all the first characters from caps to lower
    ...
    $email2 = ucfirst(strtolower($email2));
     $_SESSION['reg_email2'] = $email2;//session variables storing email
    ...
    if ($email == $email2) {
        if (filter_var($email, FILTER_VALIDATE_EMAIL))  {        // WE check if the email
            ...
            $em_check = mysqli_query($conn, "SELECT email from users where email='$email'"); //if it returns something it means that email is usd in the account
    ...
    if (empty($error_array)) { //if error_array is empty meaning there is no error recorded then it should proceed to encrypt the databs
       //$password = md5($password);
        ...
        if ($rand == 1) 
        $profile_pic = "assets/images/profile_pics/default/head2.jpg";
        else if ($rand == 2) 
